...
players well). Reports are posterior estimates: each tool is pulled toward the
usual grade for his group (src/scouting/priors.ts, generated by scripts/scout-priors.ts;
rerun it after changing the generator or development). The UI always shows the user's view. Errors are deterministic hashes, so
